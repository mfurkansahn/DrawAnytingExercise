...
public class PaintableCanvas : MonoBehaviour
{
    ...
    [SerializeField] float BrushScale = 0.1f;
    [SerializeField] float BrushWeight = 10f;
    ...
    Color? ActiveColour; /*= Color.magenta;*/ /*now should be nullable*/
    ...
    void Update_PerformDrawing(EPaintingMode PaintingMode)
    {
        ...
        if (Physics.RaycastNonAlloc(DrawingRay, HitResults, RaycastDistance, PaintableCanvasLayerMask) > 0)
        {
            ...
            PerformDrawingWith(ActiveBrush, ActiveColour.Value, HitResults[0].textureCoord);
        }
    }
